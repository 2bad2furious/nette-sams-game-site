...
abstract class BasePresenter extends Presenter {
    ...
    public final function checkRequirements($element) {
        ...
        if (!array_diff($roles, UserManager::ONLY_GUESTS)) {
            $title = UserManagementConstants::ACTION_FOR_GUEST_ONLY;
            $additional = sprintf(Translator::instance()->translate(UserManagementConstants::ACTION_FOR_GUEST_ONLY_ADDITIONAL), $this->link("Auth:logIn"));

            //if user is logged and need verification
        } else if (!array_diff($roles, UserManager::ONLY_VERIFIED) && $this->getUser()->isLoggedIn()) {
            $title = UserManagementConstants::ACTION_FOR_VERIFIED_ONLY;
            $additional = sprintf(Translator::instance()->translate(UserManagementConstants::ACTION_FOR_VERIFIED_ONLY_ADDITIONAL), $this->link("Profile:resend"));

            //if user is not logged
        } else if (!array_diff($roles, UserManager::ONLY_VERIFIED) || !array_diff($roles, UserManager::USERS)) {
            $title = UserManagementConstants::ACTION_FOR_USERS_ONLY;
            $additional = sprintf(Translator::instance()->translate(UserManagementConstants::ACTION_FOR_USERS_ONLY_ADDITIONAL), $this->link("Auth:logIn"), $this->link("Auth:signUp"));
